...
I will develop several tools that analyze and check LaTeX text documents: a search for bibliographic entries (which are used, how many times and where); a counting tool for words, characters and quotes per document; a validator of TEX files to solve issues; and a tool to import entries from TEX files.
