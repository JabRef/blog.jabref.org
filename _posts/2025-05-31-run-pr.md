...
Now you can run a PR using `gg.cmd`.
Initially, this might take several minutes, as the build environment has to be initialized and JabRef is built completely from scratch.
In subsequent runs, the process is much faster.

Windows:

```terminal
.\gg.cmd jbang just run-pr 13182
```
...
Note: Replace `13182` with the PR number of whichever PR you want to try out.
